function exp_al_with_cv_KTED(dataset, exp_settings)
disp(dataset);
%======================extract X, Y===========================
...
%======================algo setup===========================
paramCell = buildParam_KTED(tradeCandi);
%======================algo setup===========================
...
parfor i1 = 1:nRepeat
    ...
    for i2 = 1:nParam
        ...
        for i3 = 1:nFoldCV
            ...
            X_te_cv = X(te_cv_idx,:);
            Y_te_cv = Y(te_cv_idx);
            %------------------------------------cross validation for KTED
            K=myKernel(X_tr_cv, kernel_param);
			[randIdx,~] = KTED(K,param.trade_param,50);
            for i4 = 1:nBatch
    ...
    parsave(fullfile(saveDir,['KTED_split',num2str(i1),'_',dataset,'_best_Para_',num2str(kernel_param),'.mat']),res_i1_ps);
end
end
end